...
class SolverWrapper(object):
  ...
  def get_all_files(self, filepath):
    ...
    for f in os.listdir(filepath):
      if os.path.isfile(os.path.join(filepath,f)):
        ...
        files.extend(self.get_all_files(os.path.join(filepath, f)))
  ...
  def train_model(self, max_iters):
    ...
    self.net.train()
    self.net.cuda()
    bg_file_names = self. get_all_files('/media/smallflyfly/Others/VOCdevkit/VOC2012/JPEGImages')
    ...
    while iter < max_iters + 1:
      ...
      blobs = self.data_layer.forward(bg_file_names)
      # print(blobs)
      # fang[-1]
      ...
      if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, summary, front_2_1_points_loss, \
            front_2_2_points_loss, front_center_loss,back_2_1_points_loss, back_2_2_points_loss, back_center_loss, \
                center_loss = self.net.train_step_with_summary(blobs, self.optimizer)

        
        # for _sum in summary: self.writer.add_summary(_sum, float(iter))
        # Also check the summary on the validation set
        blobs_val = self.data_layer_val.forward(bg_file_names)
        summary_val = self.net.get_summary(blobs_val)
        # for _sum in summary_val: self.valwriter.add_summary(_sum, float(iter))
        last_summary_time = now
      else:
        # Compute the graph without summary
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, front_2_1_points_loss, front_2_2_points_loss, front_center_loss,\
          back_2_1_points_loss, back_2_2_points_loss, back_center_loss, center_loss = self.net.train_step(blobs, self.optimizer)
      ...
      if iter % (cfg.TRAIN.DISPLAY) == 0:
        ...
        print('iter: %d / %d, total loss: %.6f\n' % (iter, max_iters, total_loss),
              ...
              '>>> loss_cls: %.6f\n' % (loss_cls),
              '>>> loss_box: %.6f\n' % (loss_box),
              '>>> front_2_1_points_loss: %.6f\n' % (front_2_1_points_loss),
              '>>> front_2_2_points_loss: %.6f\n' % (front_2_2_points_loss),
              '>>> front_center_loss: %.6f\n' % (front_center_loss),
              '>>> back_2_1_points_loss: %.6f\n' % (back_2_1_points_loss),
              '>>> back_2_2_points_loss: %.6f\n' % (back_2_2_points_loss),
              '>>> back_center_loss: %.6f\n' % (back_center_loss),
              '>>> center_loss: %.6f\n' % (center_loss),
              '>>> lr: %f' % (lr))
        
        ##### tensorboard see loss ######################
        writer.add_scalar('loss_cls', loss_cls, iter)
        writer.add_scalar('loss_box', loss_box, iter)
        writer.add_scalar('front_2_1_points_loss', front_2_1_points_loss, iter)
        writer.add_scalar('front_2_2_points_loss', front_2_2_points_loss, iter)
        writer.add_scalar('front_center_loss', front_center_loss, iter)
        writer.add_scalar('back_2_1_points_loss', back_2_1_points_loss, iter)
        writer.add_scalar('back_2_2_points_loss', back_2_2_points_loss, iter)
        writer.add_scalar('back_center_loss', back_center_loss, iter)
        writer.add_scalar('center_loss', center_loss, iter)
